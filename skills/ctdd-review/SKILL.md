...
8. **Red-state evidence.** For every new test in the diff: is there a captured failing run from *before* the implementation (`docs/plans/<plan>.redstate.log`, verifiable with `scripts/check-redstate.py`)? A new test with no captured red state is unvalidated as a detector — it may pass vacuously. Absent evidence is a finding; an explicit "could not capture, here's why" is acceptable, a silent omission is not.
