...
4. **Draft the decision inputs.** Enter: step 3 did not fire 3.6. Emit: draft content held for step 5. Continue: after 4.5.
   ...
   3. Read `references/adr-rules.md`. Draft the structural ADR inside the future plan when the change decides a service boundary, data ownership, cross-service protocol, or persistence structure.
...
7. **Apply approved artifacts and create test evidence.** Enter: step 6 printed the Approval record. Emit: contract, ADR, tests, pin-state log, red-state log. Stop: 7.2, 7.8, 7.12.
...
9. **Produce the review packet.** Enter: step 8 produced current-turn results. Emit: Review packet. Stop: 9.1.
   1. Stop for the human's sealed hold-out result when the plan says `required`. Resolve it to `passed`, `failed`, `declined by human`, or `NOT RUN — <reason>`; only the human declines, an unavailable runner is `NOT RUN`, and `failed` blocks.
   2. Set Back-translation to one sentence derived from the changed tests, or to `n/a — no test diff`.
   3. Re-run the checkers and assemble the packet as `references/execution.md` specifies.
...
10. **Write a colocated note only when triggered.** Enter: step 9 printed the packet. Emit: Colocated note, or nothing.
   ...
   2. Read `references/colocated-notes.md`. Write one Colocated note for one universal rule, deliberate gap, or durable external fact.
