...
| The approved line says `red pause: pause` and step 7's evidence is verified | Print `git diff <diff-base> --stat`, the full diff (to a file beside the plan when long), and the intended change — one line per existing production file the implementation will modify. Order the lines by the behavior-flow step each serves, that step and its green test in front, cleanup last; name any flow step no line serves. Each line carries the current code it targets, copied verbatim, line-numbered; `—` for new content; a target too vague to copy is the finding. Under `phased`, also propose the phase map — groups from the planned paths in the repo's own structure, dependency-ordered, no architecture assumed — finalized by the release message. Ask to proceed as a Decision prompt. A requested change is an 8.6 amendment; proceed only on an affirmative human message. |
| The line says `red pause: phased` and a group's implementation is complete | Print the group's diff and its evidence — build result, every pin green, the intended-red set changed only by tests the group's own files serve — and ask to continue as a Decision prompt; recommend nothing. An in-scope correction re-enters at 8.1; out of scope reopens the gate under 8.6. The last group's checkpoint may fold into 9.4. |
...
3. Re-run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/check-spec-surface.py" --git <diff-base>` and derive the packet's `Deviations:` field: one entry per diff file absent from `Files likely to change`, per planned file untouched, per slice or 7.12 line landed through a different approach, and per 8.6 amendment (old → new in one clause), each `<subject> — <delta> — <reason>`; `none` only when all are clean. Disclosure is not authorization: out-of-plan scope reopens the gate under 8.6.
