...
| 8.7 stops with `red pause: phased` and a phase's implementation is complete | Print the phase's diff and its evidence — build result, every pin green, the intended-red set changed only by tests the phase's own files serve — then the next phase block, or that none remains, and ask to continue as a Decision prompt; recommend nothing. An in-scope correction re-enters at 8.1; out of scope reopens the gate under 8.6. The last phase's checkpoint may fold into 9.4. |
