...
| The approved line says `red pause: pause` and step 7's evidence is verified | Print `git diff <diff-base> --stat`, the full diff, and the intended change — one line per existing production file the implementation will modify; no stub shows those. Ask to proceed as a Decision prompt. A requested change is an 8.6 amendment through step 6; proceed only on an affirmative human message. |
