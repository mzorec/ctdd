...
Loaded from `SKILL.md` step 7. The evidence-state and break-point tables are
lookups: `SKILL.md` names the states so the agent can recognize one; this file
carries the required action for each.
...
2. Re-run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/check-redstate.py" <red-log> --tests-from <plan-path>` when the plan names new-behavior tests, and
   `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/check-redstate.py" <pin-log> --tests-from <plan-path> --expect-pass` when it names preservation pins.
...
`Business requirement: <text>`; `Back-translation: <text or n/a — no test diff>`; `Plan: <repo-relative path or n/a — trivial>`; `Approval: <quoted approval or n/a — trivial>`; `Plan check: <final checker line or n/a>`; `Red state: <final verdict or n/a — plan declares none>`; `Pin state: <final verdict or n/a — plan declares none>`; `Spec surface: <Verdict line>`; `Verification: <command => result or NOT RUN — <reason>>; ...` for the contract validator, focused tests, broader suite, and build; `Hold-out: <passed, failed, declined by human, or not required>`; `Residual risk: <text or none>`.
