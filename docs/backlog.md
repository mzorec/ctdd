...
**Why you'd want it fixed.** An authz matrix whose whole point is "who can reach this" that silently omits "and here is who *can*" is a comfortable lie on exactly the endpoints with the strictest requirements — the ones where AND-ed scopes appear. For eDavki-adjacent work that is the wrong place to be quietly uncovered.
