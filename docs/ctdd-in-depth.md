...
- **A claim to *preserve* behavior needs a detector as much as a change does** — and "the existing tests are the guard" is an assertion until it names *which* tests assert the behavior being preserved. Where a refactor replaces an existing implementation, the cheap procedure is to write the tests against the **old** implementation first and watch them pass, then convert; the same tests must still pass. Same tests, other order, no extra cost — and where the old implementation is deleted, that run is the only surviving record of what it did, which is why it is captured rather than narrated.
...
| #4 Untested load-bearing behavior | `ctdd-change` guardrail: thin coverage is flagged, characterization tests first (marked `currently_*` as observations, not intent), and *preservation* claims must name their detector; `ctdd-review` escalates diffs in thin-coverage areas; Pact at service boundaries |
