...
Three defects from the 2026-07-30 eDavki session, one of them mine from 0.24.0.

### Fixed
- **The duplicate-name fix broke ordinary red state.** A .NET failure block names the test twice: `Failed <FQN> [4 ms]`, then `at <FQN>() in ...` in the stack trace with no marker. 0.24.0 required *every* occurrence to agree and counted the unmarked stack line as dissent, so a genuinely failing test reported `RED STATE NOT VERIFIED` — twice in that session before the agent worked around it by reverse-engineering the matcher. An unmarked mention is not a dissenting verdict; it is not a verdict. Only marked occurrences are now compared, and disagreement is reported only among real verdicts. Four regression tests, including one proving genuine mixed results are still caught when a stack trace is also present.
- **Step 9.4 had two defensible readings and the agent took a different one each session.** It said *"Invoke `ctdd-review` on the final diff for an independent verdict, and never state that verdict yourself."* Both halves are satisfiable only in a separate context, but `invoke` reads naturally as *load the skill here* — so on 2026-07-27 the agent refused to self-review and asked for a subagent or the human, and on 2026-07-30 it loaded `ctdd-review` into its own session and issued the verdict on its own diff. Now: dispatch to a separate context — a subagent, or the human — and relay the verdict verbatim; never load `ctdd-review` into the writing session. Loading a different procedure into the same context is not independence.
- **`python3` is a dead stub on many Windows installs.** The old `ctdd-review` carried a `python`/`py` fallback; the v0.23.0 rewrite dropped it, this review flagged the loss, and the 2026-07-30 session then lost time to it — every checker call needed the full `python.exe` path. The fallback is back, in the guardrails where all three skills reach it.

## 0.24.0 — 2026-07-27

Everything here comes from one real change running end to end (finding #56) rather than from a review. The gate mechanisms held; the plan format and one checker did not.
