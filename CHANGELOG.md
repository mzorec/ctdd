...
Subtraction pass on `ctdd-change` — no rules removed, ~392 tokens of rationale removed. The skill had grown roughly a third since the v0.9.4 coherence audit across a day of individually-justified edits, which is the accretion the freeze exists to prevent: at some point instructions compete with each other for the agent's attention, and the whole case for the deterministic layer is that prose has a ceiling.

- Cut a paragraph explaining *why an earlier version of the plan-output rule was wrong*. That is changelog material and it is already in the changelog; an agent needs the current rule, not its history.
- Cut the justification for kebab-case filenames — the example beside it already demonstrates the convention.
- Removed a duplicated "say where the file was written" inside the same step.
- Compressed, without losing the causal chain: the plan-mode scratch-file explanation, the gate's honest-scope note (it catches wrong *direction* only), and the red-state verdict rule, which restated a command given two sentences earlier.
- Every rule survives: the write-before-plan-mode ordering, repo-file authority, filename convention, print-in-full, trivial exception, `CTDD-Plan:` pointer, red-state capture and verdict, the pin exemption, and pin capture.

## 0.10.3 — 2026-07-19

- **Preserve before you create.** When a change contains both preservation work and new behavior, the plan format now states the ordering rule: pins describe how the *current* code behaves, so a pin written before a change that reshapes what it observes still passes while proving nothing — pin, convert, verify green, then build the new behavior on top. Framed as a correctness property rather than a preference, because getting it backwards fails silently: the suite stays green and the detector is gone. One clause on the existing proposed-tests bullet; deliberately **not** a mandatory sequence section, since a real change derived this ordering unprompted and most changes have no such constraint to state.

## 0.10.2 — 2026-07-19

- **Red-state and pin evidence is now the checker's verdict line, not the log file** (pilot finding #26). A real change captured all three logs correctly and still shipped a silent test swap: the plan named `ReportsTotalCountAcrossAllPages`, the implementation contained a differently-scoped test — six planned, six failing, not the same six — and the dropped one asserted that the total count spans all pages rather than the current one, the classic paging defect. `check-redstate.py --tests-from <plan>` reported `not found in the log` in seconds when run afterwards; in-session it was never run. Capture had happened, verification had not: the third instance of *the mechanism exists and is not invoked* (cf. Windows no-op scripts, uncaptured pin evidence), so per finding #25 the fix is mechanical rather than firmer prose. A log proves only that a run happened; a verdict line is either present or absent, and it is what surfaces a test renamed or swapped between plan and implementation.
- `ctdd-review` dimension 8 now requires that verdict, and instructs the reviewer to run the check themselves when only a log is offered.

## 0.10.1 — 2026-07-18

- **The plan file must be written *before* entering plan mode** (pilot finding #25). Plan mode restricts writes to its own scratch file, so an agent already inside it cannot create `docs/plans/<n>.md` — and on a real change it did the only thing it could: wrote the plan to `~/.claude/plans/<random-slug>.md` and showed a summary, leaving the human asking "did you make a plan, I don't see it." That file is not the plan: disposable, outside the repo, invisible to `ctdd-review` and to CI's `CTDD-Plan:` pointer, gone with the session. Step 6 now sequences it explicitly — assemble, write to the repo, *then* suggest plan mode — and tells the agent that if it is already inside plan mode with no repo plan file, it must say so and ask the human to exit long enough to write it, rather than quietly adopting the scratch file as the record. The authority line names the trap: the repo file is authoritative, never the harness's own.
- Note on why v0.9.4's coherence audit missed this: it fixed the *rule* (the plan file is the gate's artifact, not a post-approval copy) but not the *ordering* that makes the rule achievable. Sharper prose does not help when the environment forbids the action at that moment — check what the tool permits before rewriting the instruction.

## 0.10.0 — 2026-07-18

The plan summary becomes a thing a human actually reads (pilot finding #24 — the most serious finding of the pilot, reported from inside: *"too much to read, we don't read it"*).

- **The decision summary is now written in voice, not fields.** Lead with what surprised you or what you refuse to guess at, as you would tell a colleague with thirty seconds of their attention; if nothing about the change is surprising, say that in one sentence and stop. v0.9.9's categorical strip was scannable but hollow — *"not a lot of information, and the text is so mechanical"* — because fields carry categories, not insight, and a form-shaped summary gets read as a form. The strip survives, demoted to one closing line (`Risk: · contract: · ADR · hold-out:`), which is where genuinely categorical facts belong.
- **The full plan now prints to the terminal as well as the file**, summary first and detail below — reversing the pointer-only rule of v0.8.1. That rule solved the wrong problem: detail in a terminal was never the issue, detail *without a summary in front of it* was. Composed once, with the file authoritative, so there is still exactly one plan. Plan mode's exit presentation is that file's content rather than a pointer.
- Paid the step-6 trim debt named in the v0.9.4 coherence audit: the commit-vs-ignore rationale duplicated the README and is now one clause.
- Rationale: the gate's premise is a human reviewing intent while it is cheap to change. A plan nobody reads makes it a rubber stamp, which the method's own kill criteria already call theater. The transcripts show what works — direct questions got answered immediately and changed the work; 200-line plans got approved and objected to afterwards.

## 0.9.9 — 2026-07-18

- **Plan decision summary opens with a verdict strip** (pilot finding #23, from real use across ~4 plans): `Risk: normal · Contract: additive (no breaking change) · Hold-out: required — 2 sealed tests from you · ADR: 0005 required`. The summary previously carried the *decisions* but not the four facts that determine whether the detail needs reading — risk level, breaking-vs-additive, the hold-out obligation, and whether an ADR is owed — so reviewers read the detail anyway and the two-layer structure bought nothing. Each field is a verdict; the sections below keep the reasoning. `Contract: breaking` must be stated explicitly whenever an existing response shape, route or error code changes: a breaking change discovered in the file list rather than the summary is the failure this prevents. Anything the human is on the hook for is named here, because those are commitments rather than information. The summary's test is now explicit in the skill: **a reader who agrees with it can approve without scrolling.**
...
- **`ctdd-change`: the nothing-on-disk rule now names its own exception.** It read "nothing is written to disk until the plan is approved" while step 6 requires writing the plan file — and the ambiguity had an observed consequence: the a mapping source generator session deferred persisting the plan until after approval, exactly what the file-backed design exists to prevent. The rule now says *no work product* pre-approval, and the plan file is the gate's artifact, written before approval so the human reviews it in an editor.
