...
- **The canonical example's `Existing behavior` and `Behavior flow` are written as explanation, not inventory.** A real plan followed the two-bullet example faithfully into nine by-file bullets — three opening with the same 107-character path — that read as a lookup table; its flow carried three facts in one step, opened three consecutive steps with the same condition, and held *what is untouched* inside the numbered walk as step 7. The example now shows a framing sentence before each list, plain words before identifiers, every preservation pin named in the sentence describing the behavior it protects, the one unpinned behavior called out in words, `(changed from N)` on every changed step, and effect before mechanism. The example is the operative instruction — the preamble says anything it demonstrates is not restated — so the example is where agents learn the shape; a rule saying *write prose* would not have done it. At two bullets, by-file and by-behavior were the same list, which is why the example never had to choose.
- **Rule 12 lost what the example now shows and gained what it cannot.** *Two blocks in order, `Current flow` then `Flow after change`, execution order, full sentences, a step per observable stage* is all visible in the example and came out. The fork convention — state a condition once before the steps it governs rather than opening each with it — went in, because the example's flow has no branch to demonstrate it on. Net −10.
- **Route ratchet 47,900 —> 48,700, intent-born, after the search the counsel beside it asks for.** +802 net. One restatement found and displaced (rule 12, above); `worked-change.md` holds no copy of the example; the gate-visible echo is the guarded deliberate one. Nothing else in the route restates the example, so the remainder is the example itself, and the limit moves. Recorded beside the number.
- **Four guards on the properties that made it readable, four probes firing.** Every pin in the lane is named in `Existing behavior`; the unpinned behavior is said in words; a framing sentence precedes `Current flow`; no bare `(changed)` survives. An example drifts back toward the shape an agent finds easiest to emit — a list of files — so the shape is pinned. **415 tests.** The in-depth status pin remains open from 0.49.0.
- **Rule 14 states the branch the example cannot show.** The example calls out one unpinned behavior in words; it cannot show the case where every behavior is pinned and there is nothing to say. A prose example carries more copyable surface than bullets, and a `no pin` sentence is the shape most likely to be reproduced where it is false — the one degradation this rewrite adds, named before any plan showed it. The preamble puts two-branch decisions in the rules, so the counter-branch is one sentence there rather than a second example in another domain, which would have cost ~800 characters to mitigate the thing the evidence says does not leak: the real plan copied the old example's shape and not one of its words. Guarded; the probe deletes the rule.

## 0.50.0 — 2026-09-03

- **The spec handover contract gained three requirements from the first real brainstorm, then lost the two a review round added.** Running `brainstorming` → `ctdd-change` on a real migration produced content the handover bullet had no slot for, so it gained: **measured facts about the system being changed, each with how it was established** (seven measurements moved decisions or scope on that change; a production statistic has no home in any plan section — not `Existing behavior`, which is an inventory of code, not `Assumptions`, which is by definition not measured — and with one spec decomposing into three plans, each plan otherwise re-derives its own and they drift); **options extended to positions taken and then overturned, and what overturned them** (three concrete overturns, and the reasoning that defeated a recommendation is the part no later reader can reconstruct); and **out-of-scope entries carrying why each item is out** — finding-born, not predicted: one exclusion went out, back in, and out again inside a single session, and what settled it was a discovered filter rule, which is exactly the reason that makes an exclusion survive.
- **The keep-out rule's qualifier was ambiguous, and a live spec violated it.** `behavior flows, file lists, function or test names, table columns, and anything else about code shape that the spec would be **guessing**` attaches its restrictive clause to the final item, so the first four read as unconditional — and the spec that prompted the change names a *test* in its constraints table, which the rule as written excludes flat. Fronting the qualifier as *anything the spec would be guessing* was tried and rejected: a spec saying *the change goes in `X.ToDomain()`* is not guessing either, so that reading licenses precisely the `Implementation slices` duplication the rule's own next sentence names as the harm. The line is **not** guessing-versus-known but **shape of code that does not exist yet** versus **naming an existing artifact a requirement, constraint, decision or measurement is *about***. Sorted against fourteen cases — the ten code-level facts in that spec plus four negatives — the shipped wording sorts 14/14 and the fronted one 13/14.
- **Two additions from the same rounds are deleted rather than kept.** An **exception for a code-level fact forced by a discovered constraint**, and a **test for that exception**, are both gone: an empirical count over the spec that motivated them found ten such facts — five covered by *constraints, each with its source*, five by the corrected keep-out wording, and **zero** needing the exception. An exception to a keep-out rule is how keep-out rules die, and this one was redundant twice over before it was subtle. **Revising a spec is a diff, not a rewrite** is also gone — correct content, wrong place: rule 4 (a mechanical check described in prose), rule 5 (the same discipline already stated for this repo's own docs), and no invoker, since `brainstorming` belongs to another plugin and no CTDD skill authors specs. Filed in `backlog.md` with its trigger — a second spec revision losing content — rather than dropped.
- **The always-loaded body ends at 3,913 characters, below the 5,054 it started the session at, having peaked at 7,287.** The `brainstorming` handover falls from 56% of the body to 18%, and the routing table is dominant again in a skill whose description says it routes and writes nothing itself. The contract is not smaller — 2,766 characters, and it grew by a bullet in this release — it is simply no longer paid for on every invocation that only needed to know which workflow owns a request. Headroom against the 15,000-char compaction proxy: 11,087.
- **The contract moved to `references/spec-contract.md`, and gained the element it was missing.** Checking the shipped contract against the spec that produced it found three of the spec's nine sections required by no bullet — cutover, decomposition, and owed-by-others — and one of the three bites: `ctdd-change` gates one change at a time, has no step that notices a business requirement covering three, and reads the document rather than the conversation, so an unstated split means the first plan swallows the lot or silently drops the rest. A grep of `ctdd-change` confirms no decomposition handling exists; step 6.2 draws a scope boundary *inside* a plan, which presumes the plan is the right unit. One bullet added, 270 characters — and adding it fired the split trigger filed hours earlier (*the spec contract grows again*), so the split was done rather than letting a trigger fire quietly. `SKILL.md` keeps the routing decisions and a pointer; the contract loads only once brainstorming is the route. `RoutingSkillStructureTests` guards the move in both directions and enumerates the contract's elements, because the reference-holds-only-craft guard that exists for `ctdd-change` is scoped to that skill — a new routing reference would otherwise have shipped unchecked, which is the `worked-change.md` failure that guard's own docstring describes.
- **The new guard protected the keep-out rule and not the exemption that makes it survivable.** `RoutingSkillStructureTests.ELEMENTS` enumerated *Keep out the shape of code that does not exist yet* but not *Naming an existing artifact — is not this*, so the sentence that replaced the ambiguous `guessing` qualifier could be trimmed with all 378 tests green — reverting the keep-out to absolute and silently reopening the hole this release closed. Found by probing all four new guards rather than reading them: four fired, the fifth did not. Both halves are now enumerated.
  One cosmetic residue, recorded rather than fixed: deleting the reference file is caught by `setUp` raising `FileNotFoundError` before the bundled-file assertion runs, so it fires with the wrong message. It fires.
- **Process note, since the rule is explicit about it.** Rule 3 requires an ask to state what a skill-prose change displaces and what it costs against the budgets. Two rounds gave evidence and no budget numbers; the numbers above arrived only when a third round measured. Those rounds also demonstrate the rule's own warning that reviews are an exhausted source: across two review rounds on this change, one finding was wrong on facts the reviewer could not see, one was right but out-argued by a count, and one identified a real defect with the wrong repair. The next input on this skill should be a real-use finding.

## 0.49.0 — 2026-09-03

- **The plan revision counted the edits the skill itself mandates after approval.** 7.12 says to append the intended-change table to the plan as `## Intended change`, *dating it after approval — a record of what will be built, **never part of what was approved***. Rule 7 says to replace `result: pending` before the packet, and rule 8 to list the human-verified expected values on a decline. All three land after the Approval record is written, and the digest was `sha256` over every byte — so **obeying the skill invalidated the approval it had just recorded**, and two real changes each spent a turn writing a note explaining that nothing approved had changed. Not a friction complaint: the prose asserts that section is outside the approval and the mechanism asserted every byte was inside it, so one of the two was wrong, and *never part of what was approved* is the deliberate one. `approved_region()` now normalises those three, both digest call sites go through one definition, and a required section written below `## Intended change` is reported rather than silently exempted — that section runs to EOF because under `phased` its body carries `## Phase N` headings of its own.
  Rule 13 was **not** included, though the proposal named it: *the human flips it at the gate* happens before the Approval record exists, so it is not a post-approval mutation and exempting it would have excused a field the human decides at the gate.
- **Third instance of one defect, so the fix is the discriminator rather than the instance.** `DUPLICATED sections` counted a section *name* anywhere a line began with one, so any line merely labelled with a section name read as a second section. (1) A categorical line ordering its fields differently counted as a second `risk level` — patched by excluding that one line. (2) A decision-summary line labelled with a section name; the format's own example passes **only** because its labels are abbreviated far enough to evade the detector, which nothing had recorded. (3) `Current flow, unchanged adjacent paths` — a heading the format demonstrates — rejected a valid plan as corrupt. A latent fourth fell out of the same rule: a plan carrying two `BLOCKING` questions, which the format positively invites. Two sections are now the same section when the text **after** the name is the same; a line continuing into different words is a different label.
  **A shape rule was tried first and deleted the check.** `Existing behavior (openapi.yaml; CaptureTests.cs):` is a heading form the example demonstrates, so *a heading is nothing but its own name* excluded the real headings along with the labels and a spliced duplicate passed at 18 of 22. Its own control test caught that, which is why the control is now written as the corruption that actually happened — a failed `sed` splicing a block in, which duplicates a heading verbatim. Known limit, recorded rather than left to be found: two same-name headings with *different* trailing text are not flagged. Narrower than the four false-positive classes it replaces, and unlike them it cannot reject a valid plan.
  The equality rule also surfaced a latent extraction defect. Every pattern opens `^\s*` and `\s` matches newlines, so under `MULTILINE` a match can **begin on the blank line above** the one carrying the name: `m.start()` was not on that line at all and the extracted line came out empty. The old count tolerated it by only counting matches, so it worked by accident.
- **Two of six rule-8 probes were vacuous, and the fix was the justification, not the code.** The comment claimed values are normalised rather than dropped so a deletion stays visible. Deletion moves the revision either way, because the blank line left behind is itself a difference — and so does duplication, for the same reason. What only normalising catches is an edit to the **label**: these rules mandate replacing the value, not rewriting the field, and an agent regenerating the block can easily emit a different casing or bullet. Three tries to find a case that distinguishes them; the comment and the tests now say the one the tests establish.
  The line-bounding fix is **kept and marked unguarded**, because a probe showed every duplicate test passes with it reverted: `rest` already derives from `m.end()`, and what the old bounding broke is the categorical-line exclusion reading an empty string, which no realistic plan makes observable. Filed with its trigger rather than claimed as guarded — a correct line of code with no guard, recorded so a future reader does not delete it as dead. **407 tests, 5 of 6 probes firing and the sixth recorded as expected-vacuous.**
- **Considered and not built, with its reason: exempting the hold-out where the expected values come from an approved artifact.** Proposed from the same session, on the grounds that a hold-out transcribing an accepted ADR is not independent evidence. Declined on three counts. The direction of travel closes this escape rather than opening it — v0.43.0 struck `defer` from 9.1 as the identical escape under another name, and v0.46.0 added the `result:` check after a session answered `Defer / NOT RUN`. Rule 8 already provides the middle tier the proposal reaches for, and it fired correctly on the change that prompted this: the human declined and four values were recorded as owed. And the premise is weakest exactly where it matters, because an ADR records the *rule*, not the values — transcribing a rule is where a misread lives, which is what the hold-out exists to catch. Decisively: the hold-out is at **0% execution over the whole pilot** with a redesign filed and unchosen, so an exemption would make 0% the design rather than the finding. Filed as a fourth option under that entry.
- **Not done, said plainly for the third consecutive release:** `docs/ctdd-in-depth.md`'s status pin is still at **v0.24.0**. The release checklist requires re-pinning it. It is 25 releases stale, its prose is structurally mangled from earlier edits (unbalanced parentheses, a duplicated *not yet built*), and it describes *the three skill descriptions* when there are four — bodies now 15,475 / 16,377 / 11,626 / 5,054. Bumping the version line alone would assert v0.49.0 numbers over v0.24.0 measurements, which is worse than stale in a document about doc-to-code drift. It needs its own docs-only repair pass; recorded here because two silent skips are how it reached 24.

## 0.48.0 — 2026-09-03

- **The skill that owns test-path altitude is now consulted before the path is chosen.** `ctdd-tests` owns path discovery — its step 2 prints the exact target file path — and owns the altitude rule: cover a pure transformation exhaustively at the smallest boundary that has a contract of its own, keep one representative case at the outer boundary, two exhaustive tiers is one tier of waste. But `ctdd-change` drafted every path at 4.5 and did not invoke `ctdd-tests` until 7.5, where its output contract binds it to *the approved plan path*. **The skill holding the rule was bound to a path chosen by the skill without it**, two steps earlier — which is why three rounds of wording would not have helped. The order-free guardrail now fires on drafting a path as well as on editing a test file.
  Twice in real use that put pure-transformation assertions at an integration tier. In one change eight of nine, each paying a database round-trip for a rule that never touches the database, while nine preservation pins for the same behavior already sat at the unit tier — the plan's own text carried the contradiction and nothing read it. The human caught it after implementation both times, and the repair relocated the tests post-hoc, so their red state had to be mapped back to a pre-relocation run under the old names. That retroactive evidence is the cost this closes.
  **The rule was then made reachable from the decision, because the guardrail alone would not have fired.** Moving the invocation earlier does not move the rule. The altitude row read *the boundary **reached** needs a database, network, or broker* — which presumes a boundary already chosen, so at path-choice time it scanned as not-yet-applicable — and it sits under a heading called `When blocked` whose only pointer was step 7's run classification. Meanwhile step 2 derives the path from *convention*: "the nearest behavior-level tests, target test project configuration", and convention is exactly what produced the defect, since the nearest tests for that behavior were the integration ones. The trigger now reads `the path's boundary` (**−1 character**) and `When blocked` carries `ctdd-change`'s own framing, `Do not infer an order among these condition-triggered rules; check them at every step` (**+88**). One framing for condition-triggered rules across both skills, and it makes true a premise `test_the_two_altitude_rules_cannot_fire_together` already stated in its docstring while the sentence existed only in `ctdd-change`. `ctdd-tests` body 16,377 of 16,400; the altitude row sits at 8,218, well inside the compaction proxy.
  **Filed, then built in the same release, and the filing was a mis-pricing.** This was first deferred to Tier 2 as a rule-9 limit decision, priced against `ctdd-change`'s 28 characters of compaction slack — which the edit never spends, because it lives entirely in `ctdd-tests`, where the only constraint is the body limit and no `MUST_SURVIVE` position probe binds at all. Shipping the guardrail without this would have been half a mechanism: a rule in context that nothing directs the decision to read.
  `ctdd-review` needed nothing: test altitude is already one of its mandatory dimensions and it pulls in the complete `ctdd-tests` review section, which carries the two-boundary flag. In the recorded change the reviewer was never dispatched.
- **Rejected, with its reason: a `check-plan.py` tier check.** The first design classified each lane's `path:` values as fast- or slow-tier and fired on the asymmetry. It was a proxy, not a detector — whether an assertion is a pure transformation is not derivable from anything the plan carries: the test bullet's category names *case kind*, not boundary altitude, and the assertion does not exist yet at step 5. It also misclassifies the neighbouring change in the same file, since adding another projected column to that view is genuinely database behavior and fires identically. A checker reporting on input it cannot classify is this repo's signature defect, twenty instances deep. Rule 4 prefers a script; here the script's input does not exist, and no trigger is filed because nothing would make it appear.
- **The binding budget was the compaction proxy, not the route ratchet, and pricing against the wrong one produced the wrong shape first.** The clause was drafted at step 4.5 at +89 characters against 152 of route headroom, and it fit. It also pushed the `references/colocated-notes.md` loader at step 10.2 from 14,940 to 15,029 — past the 15,000-char compaction proxy — so a reference loader stopped surviving compaction. The real slack was **60 characters**. Moving the clause to the order-free guardrail cost **+32** and is the better home regardless: the guardrail is already a `MUST_SURVIVE` probe where a 4.5 clause is not, and it binds the 8.6 amendment that retargets a test, which is where the relocation actually happened. No limit was moved and nothing was displaced.
  The probe's anchor string was extended to cover the new trigger, so what compaction protects is now both triggers rather than the edit trigger alone; the first wording broke that anchor and two probes caught it within one run. Three probes on the new guard — dropping the path-drafting trigger, dropping the writing constraint, and moving the rule out of the order-free section into a step — each fail it. Body 15,475 of 16,400; route 47,748 of 47,900. **The next change to this body has 28 characters before that loader falls off again; that, not the route ratchet's 152, is the number to price against.**

- Backlog: the decision summary is specified three ways and only one is reachable. The skeleton caps it at one to three sentences, the gate-visible rule demands one line per refusable decision and bans bundling, and `check-plan.py` reports any summary line labelled with a section name as a DUPLICATED section. The canonical example passes only by abbreviating its labels far enough to evade that detector, which was never recorded. Attempted and reverted: the compliant form fails the linter, measured both with section names and abbreviated. Filed with two resolutions — exclude the summary from duplicate-section scanning, or retract *one line each* — because the choice is a design decision and the fix is script-side.

- `docs/companions-routing.md` is tracked, and the shipping guard is widened to the class it was written for. The file was announced in this changelog and never added — a fresh clone got the reference and not the document. Third instance of one defect: `worked-change.md`, then `execution.md`, now this. A guard already existed and could not see it, because it globbed `skills/*/references/*.md` alone — written for a class, applied to one directory. It now also checks every path named in a shipped document that exists in this repository, which is the rule that catches all three: coverage went from 9 files to 18, reaching `docs/`, `scripts/`, `hooks/` and the root. Paths naming files in the reader's own repository — the worked example's tests, `<plan-dir>/<name>.md` — do not exist here and are ignored, so `is_file()` rather than markup is what bounds it. **The first attempt keyed on backticked spans and found 834 of them in the changelog containing not one real path**: pairing `` `…` `` across a document with fenced blocks offsets every span after the first fence. The extraction is backtick-independent as a result, and asserts it found at least six paths so it cannot pass vacuously. Probed three ways — untracking the doc, a reference, and a script each fail it.
- Backlog: the steps 7–10 relocation entry is corrected rather than rewritten. Its reasoning stands; the fact it rested on decayed. Four `MUST_SURVIVE` probes were added back into steps 7–10 within five days of the 2026-07-21 measurement, so the move is no longer *strictly better rather than a trade* — it is now the same trade the entry itself argues against for step 6. Relocating those four rules to the order-free guardrails is filed as the prerequisite that would restore the original premise. Every number in the entry is re-measured and marked stale, and the trigger is recorded as fired-but-contaminated: of the two real failures, one traced to a rule defect rather than truncation, and the other is one day apart from the body crossing the compaction proxy with nothing to separate the two explanations.

## 0.47.0 — 2026-09-02

- **The pause shows the evidence lane it had already written, not just a stat.** Its two halves had opposite altitudes: work still to come got a per-file table with a descriptive sentence, per phase, plus `Consumes:`/`Produces:`; work already written got one `--stat` line and a diff in a file. Backwards, because the already-written half is the evidence lane — in the same phased run, ten files and +330/-42 carrying **fourteen test retargets and one replaced assertion**. This method calls changed test expectations changed requirements, so that was the most review-worthy content in the pause and it appeared as a number. The human challenged it and the agent then produced exactly the right artifact retroactively: a `File` / what-it-adds table tracing each row to its plan item. The row now requires that table at the pause, with the plan item each row serves — which is the question that was actually asked and took three exchanges to answer. **+119 characters**, headroom 303 —> 184; two probes.
  Checked and cleared, against a suspicion of mine that was wrong: the plan named all of it. Its `Changed existing assertions` section carries the one real assertion change with old and new forms and enumerates the fourteen retargets as verbatim-assertion renames, so rule 3 was satisfied. I had first grepped the wrong plan — the previous day's — and read zero occurrences as a planning failure. It is a presentation gap, which is why the fix is in the pause row and not in `plan-format.md`.
- **The phase checkpoint names which block to print, because `next` was misread.** The pre-registered test ran: a real phased change on 0.46.0, with the re-read clause live at all five sites. **The 7.12 pause was correct and complete** — three phase blocks, each with its `File` / `Intended change` table and `Consumes:` / `Produces:` line, then the ask. That is the format working first time after three revisions, and it retires the worry that the pause presentation is unfollowable prose.
  The checkpoints failed, in two different ways. At the Phase-1 checkpoint the agent printed a **correctly formatted block of Phase 1** — the phase that had just finished — while asking to continue to Phase 2. That passes a glance, and it actively misled the review: the human's next words were that a lot of changes were not in Phase 1 as planned, and three exchanges went into reconciling the diff against the wrong table. At the Phase-2 checkpoint no block was printed at all.
  So `next` had to stop being an ordering word: the row now says **the block for the phase being continued to and never the one just finished**, with the observed failure banned by name, because checkpoint 1 proved the agent renders the block correctly once it knows which one. **+54 characters**, headroom 357 —> 303. Three probes: reverting to `next`, dropping the ban, and leaving the referent unnamed each fail the guard.
- **What the run did not settle, recorded rather than guessed.** The Phase-2 checkpoint's silence is unexplained. It may be a knock-on of the same confusion or independent degradation, and with `next` now unambiguous a second run separates them. Also unknowable from this evidence: whether `execution.md` was re-read at either checkpoint, because this transcript export renders only `Update`, `Bash`, `Write` and `Skill` calls and no read markers at all — the same limitation that produced a wrong diagnosis two releases ago, so no inference is drawn from the absence this time.

## 0.46.0 — 2026-09-01

- **`check-adr-drift.py` claimed a pass over a diff it never read.** `read_diff` returned `subprocess.run(...).stdout` and ignored git's exit code. An unresolvable base writes to stderr and leaves stdout empty; an empty diff removes no marker; so `--git nosuchref` printed *no ADR lost its last marker* and exited **0**. Reproduced in this repo before fixing. That is the defect shape `ctdd-in-depth.md` records more often than any other — a checker reporting success over input it never read — sitting in the newest checker, five releases after it shipped. It now exits 2 carrying what git said, and the encoding is pinned so a mis-decode cannot drop marker lines the same silent way. Found by reading a real session where the checker crashed and the agent substituted a manual grep for removed marker lines as a stand-in — which has the identical hole and would also have read clean on a base that failed to resolve. The crash itself is not reproduced here and is not fixed; it needs a repository whose git root sits above the code root.
- **`check-plan.py` now judges the hold-out `result:`, which it never looked at.** v0.43.0 struck `defer` from 9.1's options because it was the identical escape under another name. A real session then answered the prompt with `Defer / NOT RUN`, and the plan recorded a result of NOT RUN, deferred by the human at packet time, explicitly not a decline. That leaves load-bearing boundary evidence neither owed nor waived, which is the state `declined by human` exists to record — and the recorded fix had been live for three releases. Three wording fixes on this class of ignored-explicit-rule have changed nothing, so this one is checked where it lands rather than where it is asked: the result must be one of `pending`, `passed`, `failed`, `declined by human`, `NOT RUN`, and may not defer. `NOT RUN` stays legitimate for a runner that is genuinely unavailable, asserted beside the rejection so the check cannot pass by failing always. Folded into the existing hold-out check rather than added beside it, so `HOLDOUT_BLOCK` stays the one definition of where that section is.
- **Both tests were green for the wrong reason first, and the fixture now records why.** `_holdout_required` reads the categorical line alone, so a fixture that replaced only the `Hold-out` section left the new check dormant and the tests passing against the defect they were written to catch. Each fix was then probed by reverting it: all three assertions fail when their check is disabled.

## 0.45.0 — 2026-09-01

- **Every step-7 and step-8 breakpoint now re-reads `execution.md` rather than trusting context.** A real session presented the phased pause as two prose bullets and the phase checkpoint in a looser format; asked afterwards, the agent said it first read `execution.md` during Phase 3 — after both breakpoints had fired — because it had the skill loaded and saw no reason to reopen a reference. Five steps order that read, and they were not phrased alike: four said `read references/execution.md.` and 9.3 said `Read ... **now even if read earlier**`. In that one run, on that one file, the emphatic site was obeyed and the packet came out exactly to spec, while the two plain sites that fired were not and the presentation they own was improvised. Same agent, same file, ~1,500 lines apart — a natural experiment rather than a theory, which is why this is a wording fix where the Decision-prompt symptom is not. 7.7, 7.11, 7.12 and 8.7 now carry 9.3's clause; the standalone-ADR route does not, because it reads the file first thing and `even if read earlier` would mean nothing there. **+100 characters**, headroom 457 —> 357. A guard asserts all five ordering sites carry it. The token cost was measured before shipping and accepted rather than discovered: `execution.md` is 1,940 tokens, so a `phased` change with four phases re-reads it six times for **~11,640 tokens**, about the whole plan-gated route again — and 8.7, the step that repeats, uses 116 of the 1,940 it fetches. Shipped as-is deliberately: the next phased change is the only way to learn whether re-reading is the mechanism, and one expensive run that decides beats a cheap one that does not. Outcomes are pre-registered in `docs/backlog.md`, including the middle case where the pause renders and the checkpoints still drift. Worth recording: only the *presentation* degraded. The evidence lanes are enforced by scripts and held throughout, which is exactly why this survived releases — and why the earlier diagnosis that the reference was never read at all was wrong: the rendered transcript collapses reads, and the plan's 22 mandatory sections proved `plan-format.md` had been read after all.
- **`ctdd-routing`, and three descriptions that decline to defer (shipped in `96c54f1` without a version bump or an entry; recorded here).** A fourth skill routes between ctdd and the superpowers phases around it — brainstorming, systematic-debugging, using-git-worktrees, finishing-a-development-branch — and names the six that are superseded with the reason for each. `ctdd-change`, `ctdd-tests` and `ctdd-review` each gained a clause refusing to defer the plan, the gate, test authorship, red state, or the verdict to an installed superpowers skill. Two costs worth stating rather than discovering later: descriptions are always in context and went **360 —> 536 words, +49%**, of which roughly half is superpowers-conditional text paid for by every session whether or not superpowers is installed; and `docs/companions-routing.md` now states the same ruleset as the new skill in different words — thirteen near-identical lines, zero identical ones, which drifts rather than duplicates. Neither is resolved here.

## 0.44.0 — 2026-08-29

- **Two of the four things the budget squeezed out are restored; two are not.** Restored because they were content: `The last phase's checkpoint may fold into 9.4`, which is a formatting permission rather than a duplicate of `or that none remains` — that clause says *what* the checkpoint prints, this says the checkpoint and the handover may be one message; and `no architecture assumed`, which on reflection is distinct from `in the repo's own structure` — the first says where to take the order from, the second says not to re-order by a layering the agent imagines, and the second is the one an agent would actually violate. Left out because they duplicated a mechanism rather than saying anything: the warning that a table or a heading empties the lane, which `check-redstate.py` prints itself on exit 2 — *Check the new-behavior heading and that names are bullets* — and the note scoping the bullet shape to pins and changed assertions, which the worked example demonstrates. Restoring prose that restates a script's own diagnostic is the opposite of what the raise was for. Net **+71 characters**, headroom 529 —> 458.
- **Route ratchet 47,400 —> 47,900, intent-born.** Asked for, so that the route stops shortening prose to fit. **+500 and deliberately modest**, restoring the ~450-character margin the v0.41.0 note says the previous ceiling held. The counsel is recorded beside the number because the raise weakens something that was working: the ceiling is arbitrary in origin, but every time it fired across v0.43.0 and v0.44.0 it found a real restatement rather than forcing a cut — `no architecture assumed` duplicated a clause two words earlier; two guidance sentences restated the worked example and a script's own exit-2 diagnostic. Nothing was lost to shortening, and the search for redundancy is what the pressure bought, so a larger raise would have stopped the search rather than unblocking it. Untouched, because they bind elsewhere: `ctdd-tests` at 16,290 of 16,400 with no reference to move procedure into, and the compaction probe at 13,614 of 13,800. The filed steps-7—10 relocation is still the only change that moves the route *number* rather than the ceiling.
- **A phase states what it consumes and produces, adapted from obra/superpowers.** Their `writing-plans` gives every task an `Interfaces: Consumes / Produces` block. Almost nothing in that skill transfers — its plan is an executable script of code blocks for an engineer with no context, ours is a specification a human approves and a checker validates — but this does. A phase table said what each file does and never what the phase hands the next one, so the chain was inferred from prose or assumed from the numbering. On the specimen that prompted it, a stored-gzip passthrough in four phases, three chain and the fourth — the client registration — consumes nothing from the others: it could run first, in parallel, or ship as its own change, and a numbered list said the opposite. It also makes the seam clause legible rather than asserted: a phase that produces an interface, a field on a type, has no behaviour to assert, so `turns nothing green` follows from what it produces instead of being taken on trust. `—` for none is required, because blank reads as forgotten and an independent phase is the finding the field exists to surface. **Displaced to pay for it:** `no architecture assumed`, which restated `in the repo's own structure` two clauses earlier. Net **+13 characters**, headroom 42 —> 29; three probes.
- **The phase checkpoint shows what continuing means, intent-born.** It printed the finished phase's diff and evidence and then asked whether to continue — with the thing being continued *to* last seen at the pause, far up the scrollback. The one decision the checkpoint exists to take was the one it gave no material for. It now reprints the next phase block, the same heading, imperative and table the pause printed, or says none remains. The unit is named once, in the pause row that specifies it, so the two surfaces cannot drift into two descriptions of one artifact — which is exactly how the pause itself ended up carrying a table and a duplicate file list. **Displaced to pay for it:** `The last phase's checkpoint may fold into 9.4`. That permission existed for the moment the new clause now states outright, so it is superseded rather than dropped for being cheap, but it is a real removal and reversible if the fold turns out to be missed. Net **+28 characters**, headroom 70 —> 42; three probes.
- **Phase headings become a label plus an imperative, intent-born — on the third try, and the first two are recorded so a fourth argues with them.** `<what it does, one sentence>` produced `Phase 4 — let the shipped client ask for it`, which names no work. `<what changes, and what is true afterwards that was not before>` produced `the coding reaches the API boundary`, a state rather than a task. The pilot could read neither, and both were invented here. Every shipped Anthropic skill converges without exception on a different shape — a 2—5 word label in the heading over an imperative body, `## Step 2 — Technology fingerprint` above `Identify, with file evidence:` — so the row now requires that, with the imperative ending in what the phase turns green or that it turns nothing green. That seam clause is adjacent to a `Turns green` column v0.42.0 dropped as unread, and is kept deliberately: a per-file column competing for table width is not a phase-level statement of why the boundary exists, and it is what tells an approver that the checkpoint after phase 1 will have no passing test to show. Both rejected forms are now banned by name.
- **`existing production file` becomes `the change touches, new files included`.** Correct while the table stood beside a phase list that named every file; wrong once the table became the phase's whole contents, because new files then appear nowhere — a phase adding a helper would be described entirely by the files that route through it. Caught by rendering a real pause against the new spec rather than by a guard. **Net +122 characters across both changes**, headroom 192 —> 70; five probes, one per clause.
- **The pause becomes one table per phase, intent-born.** With the table restored, the real shape problem showed: it and the phase list carried the same filenames, with the descriptions on only one of them, so the reader cross-referenced to learn what a phase did — exactly the cost v0.42.0 said the phase list existed to remove. It added a second list instead of merging. Now `## Phase N` headings each carry their own `File` / `Intended change` table, and no separate file list is printed beside it. `Flow` is dropped: on a real change seven of ten rows shared one step, so the column ordered almost nothing, and the coverage check it anchored rests on the unserved-steps line, which the guard now asserts is still there — counsel, since that line is the whole check. The phase heading also gains a real demand: `what it does, one sentence` produced `Phase 4 — let the shipped client ask for it`, which names no change; it is now `what changes, and what is true afterwards that was not before`. **Net —40 characters**, headroom 152 —> 192, because the merged spec is shorter than the two it replaces. Also observed and not yet solved: with the table restored the agent rendered it as label-prefixed vertical blocks with rule separators, which the same row bans in as many words — so the ban is not holding, and no guard can see a rendering the row already forbids.
- **Fixed, from real use: a `phased` pause printed the phase list and no table.** The row fired — the phase list only exists inside it — so this was not the trigger defect fixed earlier; it was a sentence in the row that reads two ways. *Under `phased`, follow it with the phase list rather than a table* was meant as *the phase list is a list, not a table*, and parses equally well as *under phased, the phase list replaces the table*. The agent took the second reading and the pause said what had been proved and never what would be built. v0.42.0 settles the intent: phases are a list *so the schedule is legible without decoding the first table*, which only reads if the table is still printed. The clause now names the table and negates about the list — **+29 characters**, headroom 181 → 152. A guard asserts both halves and bans the construction that misfired, because a positive assertion alone would pass on a reworded ambiguity. Nothing had guarded the two artifacts coexisting, which is how this survived a release.
- **Step 0 becomes a script: `gen-baseline.py`.** `diff-base` is not a note for the reader — it is the literal argument four later checks take (`check-spec-surface.py --git` at 3.3 and 8.4, the packet's re-run at 9.3, `git diff <diff-base> --stat` at 9.4), so a hand-derived value was a hand-derived input to every mechanical check the method has. It is now derived once and those four agree by construction. The larger half is 0.3: *stop and ask which base to use when the target branch is absent, disputed, or has no merge-base* was a condition the agent had to **notice**, and noticing is what degrades — it is now detected, and exits `1` with the choice named and nothing claimed. Inference is `origin/HEAD`, then a single local `main`/`master`; two of them is the *disputed* case and refuses rather than breaking the tie. Step 0's prose drops from four sub-items to two, **body −149 characters**, headroom 32 → 181. 17 tests, each against a real git repository rather than a mock, because the whole property is that the script agrees with git.
- **Two bugs the new tests caught on their first run, both worth recording.** `_git` did `.strip()` on command output, which is right for a single token and wrong for `git status --porcelain`, where column 1 is the index and column 2 the worktree: the leading space of the *first* line was eaten, every column shifted, and a modified-but-unstaged file was reported as staged under the path `eed.txt`. And the on-target report compared `branch == target` directly, so with the target inferred from `origin/HEAD` — the common case — `main` never equalled `origin/main` and *working directly on the default branch* went unsaid. Both are the shape of defect this whole exercise is aimed at: correct-looking prose that nobody could have checked, now checked.
- **Two guards follow the properties that moved rather than being deleted with the prose.** 0.1's on-target remark and 0.3's question moment both live in the script now; the guards point there. That is a stronger position than the prose held — a sentence in the body can be read and not acted on, where a line the script always prints cannot.
- **New guard: every runtime script must be named by a step.** The flag guard's rule one level up — a *script* nothing runs is a checker the workflow does not have. It enumerates flags, so it only ever catches a dead flag on a live script; nothing required step 0 to call `gen-baseline.py`, and reverting the step to prose failed one unrelated assertion by accident of a phrase sitting in its site list. Deliberately a presence check and not a usage check: which step runs a script is a design question, and freezing it here would be the wrong guard.

## 0.43.0 — 2026-08-27

- **A plan's test bullets become multi-line, intent-born.** `New-behavior tests` and `Preservation pins` now put the test name and its case categories on the bullet's first line, with `covers:`, `path:`, and `red:` indented beneath it, replacing one line that crammed five fields behind semicolons. A real pilot plan showed why: across ~70 tests, three sections of visually identical prose paragraphs left the reader nothing to hold — the section heading scrolls away, `covers:` (the clause the gate actually approves) sat fourth of five behind a 100-character path repeated on every bullet, and the two lanes differ only by an absent `red:` buried at the end. The new shape gives `covers:` a fixed left margin, which is a table's scannability without a table: a table, or a heading between bullets, empties the lane for `check-redstate.py` — verified, exit 2 and zero names — because the parser requires the name to lead a bullet. Net **−34 characters**, so no ratchet moved; the repeated `path:` / `case:` / `expected pre-implementation failure:` labels cost more than the newlines and indentation they were replaced with.
- **Two example guards were parsing a field that no longer exists, and the pin lane had no guard at all.** Both category checks read `case: <x>;` mid-line and are rewritten to parse the first line by position, scoped to the two test sections so the `Case coverage not reached` rows cannot contribute `n/a` as a category. One carried a hole worth recording rather than reproducing: its `[a-z ]` class excluded commas, so every multi-category bullet (`case: positive, side effect;`) matched nothing and was never checked. Both now fail on a bogus category *and* on a bullet regressed to the old cram. Added: an extraction test for the preservation-pin lane, which nothing covered — only the new-behavior names were pinned to the parser, so half of any reshape was unverifiable.
- **The gate prints the reasoning sections, intent-born.** `Behavior flow`, `Known gaps`, `Assumptions` and `Uncovered or ambiguous` now go to `stdout` in full at step 6.1 beside the summary and the `Hold-out`, instead of being named in one line and printed on request — the approver reads the reasoning at the gate rather than round-tripping for it. This reopens a twice-rejected decision and both recorded failures still stand as the bound: `print the complete plan verbatim` was unfollowable at 31,448 characters so agents compressed and lost the hold-out first, and the eight-section replacement made the gate scale with the plan. What is different is that this is a **closed set of four**, asserted by set equality so a fifth cannot be added silently, and the format already omits all four at the small tier — so the small-tier gate costs nothing. Three are bounded lists; `Behavior flow` is the one that scales and the one to watch. The two sibling guards were re-derived from properties rather than phrases: reaching the gate is now satisfied by either route, and the summary is checked for *disjointness* against what already printed, replacing an `assertIn` on the single-exemption wording that the five-way set made meaningless.
- **Drift alarm 13,700 → 13,800, intent-born.** Naming the closed set in the always-loaded Output contract row costs 65 characters the body did not have. Checked first, per rule 9: this is the alarm, not the boundary — the real survival test still passes, the last must-survive probe sits at 13,752 of the 15,000 compaction proxy, 1,248 characters of true margin. No displacement was taken and the reason is recorded beside the number: the two candidates were the summary-names echo, which a guard records as a deliberate two-surface redundancy, and a 36-character clause short of the 52 needed — taking it would have been the cheapest-item deletion rule 9 forbids. Reversible: return the row to a pointer and the offset falls back under 13,700.
- **Fixed, from real use: the approval gate stopped offering options.** The Decision prompt's destination read `interactive question when offered, else `stdout`` since v0.24.0, and *when offered* is not a condition an agent can check — so the `stdout` fallback was always reachable and always defensible. It stayed harmless until v0.37.0 added `Recommend nothing at the step 6 approval gate` to a shape whose three clauses include *one recommended with a one-line reason*: the stated shape became unsatisfiable at exactly the gate, and an unsatisfiable shape beside a sanctioned exit resolves to prose. The destination is now `interactive question; `stdout` if none exists` — the exit closes and the only resolution left is to emit the options and omit the recommendation, which is the wanted behavior, so the shape clause needed no rewording and v0.37.0's exclusion is untouched. This reached three surfaces, not one: 7.12's pause and each phase checkpoint delegate their form to the same row. Net **−3 characters**. A guard now holds the form, asserting the destination leads with the interactive question and keeps `stdout` conditional on a checkable absence; nothing had held it before.
- **Fixed, from real use: questions raised while reading the slice had no form.** A second report of *no options to choose* turned out to be a different defect from the one above, which is why fixing the destination column did not move it: that governs outputs already declared a Decision prompt, and this moment never declared one. Step 2's header says `Continue: always` — it is specified never to stop, and emits only the reading to `stdout` — so two design questions discovered by reading the code came out as prose. Step 1.1 could not catch them: it is scoped to *the business requirement* and runs before the read. `0.3`'s `Stop and ask which base to use` was unbound for the same reason. Rather than a fourth per-site repetition, one order-free guardrail now binds every question at any step, and the three existing repetitions retire into it — **net −7 characters**, headroom 8 → 15. A guard holds the rule inside the condition-triggered section, because moved into a step it becomes ordered and stops covering step 2, and names the four moments it exists to cover so renaming one cannot leave the rule covering nothing. Still open and not fixed here: the shape is written for a single question (`2–4 exclusive options`) while this moment produced two independent ones, so a multi-question ask has no stated form.
- **ADR supersession gets a guard; it had none.** Asked from real use whether a superseding ADR should be numbered `0011-2`, the answer was already in rule 16 — *numbers stay flat and chronological, never sub-numbered* — but nothing held any of the rule, and a question reaching a human about a rule that already decided it is the same signal as drift. The guard holds the four clauses across the three surfaces that have to agree: flat numbering against the Output contract's `NNNN-<kebab-slug>.md` path, lineage in the `(supersedes NNNN)` title, and *there is no amended status* against the template's `Status` enum, which is the only place an `Amended` value could appear. The fourth is the citation most likely to rot — rule 16 names the step that flips the old record's status, and steps renumber. It parses the number the rule actually cites rather than hardcoding it, so an edited citation is followed to wherever it now points. Eight probes, one per clause. **The first version failed one of them:** it asserted `"ADR" in line`, and a citation moved to step 2.1 — *read every ADR named by a marker* — walked straight through. Rewritten to test the step header's `Emit:` list, which is structural and independent of any sub-item's wording. Guards cost nothing against the route budget; `adr-rules.md` loads conditionally.
- **Fixed, from real use: a `phased` plan's pause matched no row, so it improvised.** 7.12 stops for every value that is not `skip`, but the row carrying the entire pause presentation triggered on the literal `red pause: pause`. A `phased` plan therefore matched nothing at the initial stop — the sibling row wants *a phase's implementation is complete*, and nothing is implemented yet — so the agent wrote prose and the `Flow | File | Intended change` table never printed. The pause said what had been proved and not what would be built. Compounding it, the one clause written for that lane, *Under `phased`, follow it with the phase list*, sits inside the row `phased` could not reach. The trigger is now the same exclusion 7.12 routes on — `is not `red pause: skip`` — rather than an enumeration of the values that are not `skip`: the enumeration is what rotted, and a value added later would have gone the same way. **+1 character.** A guard asserts the coupling rather than the wording: 7.12's predicate and the row's must name the same excluded value, re-enumerating fails, and the phased clause has to stay in the row that fires. Nothing had guarded either pause row.
- **The two pause rows can no longer both match.** Widening the initial pause's trigger to every non-`skip` value left it true at a phase checkpoint as well — the line is still not `skip`, step 7's evidence is still verified — so the full initial presentation could reprint after every phase, with only the sibling row's strictly-later condition informally separating them. Each row now names the breakpoint it is reached from, `7.12` and `8.7`, which is also how the agent arrives at it. **Net −18 characters**, headroom 14 → 32: the anchor is shorter than the precondition it replaces, because reaching 7.12 already implies step 7's evidence. The guard gained the disjointness check and a citation check of the same shape as rule 16's — the cited steps must exist and must actually read this file, so a renumbering cannot strand either row.
- **Not done, and not quietly upgraded:** `docs/ctdd-in-depth.md`'s status pin still reads **v0.24.0**, eighteen releases stale, and its surrounding sentence is garbled from prior editing. Re-pinning the version alone would relabel v0.24.0-era measurements as v0.43.0 ones, which rule 7 forbids; a real re-measure plus the prose repair is its own change and needs the human's call.
...
- **`check-adr-drift.py`: the quietly-moot ADR now has a detector.** Supersession covers decisions a change knowingly replaces; the uncovered case was the ADR whose governed code gets rewritten while nothing names it — step 2.1 only reads ADRs the touched files point at, so an accepted record can outlive its truth unnoticed for months. The signal is marker *removal* in the diff: if the last `ADR-NNNN` in the tree goes, the checker reports the id, its file, and its Status, and exits 1 — re-mark, disclose, or supersede. Deliberately not a whole-repo audit, which would report the same never-marked ADRs every run and teach everyone to skip the line (findings #24/#27). Wired into packet assembly, stated as rule 18, and reviewable: an orphaned ADR with no disclosure is `process-evidence`. 12 tests; ids normalize across widths and `---` diff headers can't fake a removal.
- **ADR markers become bare references, and supersession gets a deadline.** A pilot marker landed in a C# `<summary>` carrying two lines explaining the decision — rule 17 asked for exactly that (`ADR-NNNN` *and one line of what it decides*), which puts a drifting second copy in the member's own description, where IntelliSense shows it in place of what the member is. The marker is now the identifier alone, placed where the repository's comment syntax carries ancillary notes (`<remarks>`, not `<summary>`; `@see`, not the leading sentence); the reader follows the reference. Separately, rule 16 said to flip a superseded ADR's `Status` but never said when, and nothing checked — it now happens in the same change at step 7.3, and `ctdd-review` reads both a dangling supersession and a restating marker as `process-evidence`. Both files sit outside the route ratchet, so no budget moved.
- **The pause prints two tables, intent-born.** Splitting the intended-change rows under phase headings answered the wrong question: the reader lost the flow order that made the table readable, and phases were left with a thin summary — files abbreviated, nothing about what the phase does. They are separate concerns and now separate tables. The intended change is one wide table again, `Flow` · `File` · `Intended change` as a full sentence · `Turns green`, one row per production file, ordered by the behavior-flow step it serves with cleanup last, unserved steps named. Phases follow as a list rather than a table — `Phase N — <what it does>` over a bullet per file it touches — dependency-ordered, so the schedule is legible without decoding the first table and full paths are not squeezed into a cell. The verbatim-current-code clause is gone with the block format that grew out of it. `Turns green` and `Checkpoint evidence` were then dropped: the pilot does not read them and neither helps the agent — the plan's slices already name each row's green test, and the break-point row already fixes what every checkpoint prints, so both columns were restating rules that fire anyway while consuming the width the descriptive column needs. Net −135 characters; no raise.
- **The intended-change table's shape is now fixed exactly, intent-born.** Two live surfaces drifted off it: the plan's `## Intended change` came back as label-prefixed vertical blocks (`Flow step:` / `File:line:` / `Current code (verbatim):` / `Change:`) with terse fragments where the original wide table carried full descriptive sentences, and the phased terminal printed a thin second table — phase, abbreviated files, turns-green — that says nothing about what will be done. The row now specifies the artifact rather than describing it: exactly two columns, `File` and `Intended change` as a full sentence, one row per production file; no blocks, no label-prefixed records, no extra columns; phases are `## Phase N` headings over that same table, so the phase sections *are* the map and no summarizing table follows; the plan copy is the same table with one added leading `Flow` column. Ratchet 47,300 → 47,400 under the override.
- **The decision summary may not be grouped, intent-born.** A real gate returned it as lettered groups A–D with several decisions per bullet, which defeats the section's purpose: refusal becomes all-or-nothing where the format promises one refusable line each. The rule gains teeth — one line, one decision, refusable alone, never lettered groups or paragraphs bundling several — and the banned word is the same one the phase vocabulary purged. Ratchet 47,200 → 47,300 under the override; third presentation fix this release on a route with no slack, which makes the filed steps-7–10 refactor overdue rather than optional.
- **The pause presentation returns to the wide table, phases as its headings, intent-born.** Two real pause transcripts were compared: the flow-ordered, verbatim-snippet form this release introduced fragments the change into per-file blocks and printed the phase map twice, while the earlier wide `File | Intended change` table reads in one pass. The row now demands one scannable two-column table, split under `## Phase N` headings when phased — each with its checkpoint-evidence line, cleanup last, no second map — with current code cited only where a row needs it, and closing with the few things most worth the human's eye (a shape the first transcript improvised and the rule now requires). Vocabulary purged: phases are never called groups. The same rows are also appended to the plan as `## Intended change`, ordered by flow step — the terminal orders by build sequence, the file by approved behavior, so the durable artifact carries behavior and landing coordinates together; appended after the gate, it records what will be built without becoming part of what was approved, leaving format rule 12 intact, and carries one line stating its ordering and its post-approval date so a later reader needs no transcript to read it. Ratchet 47,000 → 47,200 under the owner's override.
- **Governance, intent-born: the evidence gate gains the owner's override.** The specimen-first discipline hardened across this release until it was parking the pilot's own wishes — the tool telling its owner no. The one-line test keeps its falsifiability question and gains a sanctioned trigger with full standing: `pilot intent — <reason>` files or ships immediately, needs no specimen, reopens recorded rejections, and may not be parked or argued down by any agent. Both raise bars honor it; no invocation phrase exists — a plain directive is the trigger, agents classify and record, musings stay conversations, questions address shape never whether, and counsel rides beside the work in one line. The only requirement is the mark itself — *intent-born*, in this log — so the findings loop can judge intent-born and specimen-born mechanisms by the same measure: results.
- **The categorical line gains a red pause, and step 7 gains a second gate the human chose in advance.** The plan gate approves behavior before code exists, and the next thing the human saw was a finished diff — the divergence between the mental model formed at the gate and the code that landed had nowhere to surface until it was already built. The line now carries `red pause: <pause | skip>`, decided at the gate like every other field (drafted `pause` at the large tier, `skip` at small); when flagged, new step 7.12 stops after red state is verified and prints the pre-implementation diff — the planned contracts, ADRs, tests, and compile-only stubs, *real artifacts rather than a description of them*, which is finding #28's copied-not-composed rule taken to its limit — plus the intended change, one line per existing production file the implementation will modify, because behavior-level tests are placement-agnostic by design and a modify-in-place change leaves no stub: the pilot reported implementations landing off-approach several times, and those lines are the one part of the pause that has to be composed because no artifact can show it yet. The lines order by the behavior-flow step each serves, its step number and green test in front and cleanup last — derived joins, since the flow is numbered and every slice names its test — and a flow step no line serves is named before the prompt: approved behavior nobody intends to build, caught pre-implementation. A diff that outruns the terminal goes to a file beside the plan, which the first pilot pause improvised and is now blessed. After the second pilot pause, each line also carries the current code it targets, copied verbatim and line-numbered — the change reads as before/after against the file, not against memory; `—` for new content, and a target too vague to copy is itself the finding, because it means the agent cannot say where its change lands. Deviations closes the loop against the lines. A requested change there is an 8.6 amendment back through the gate, never a quiet edit. `check-plan.py` requires the field to exist (existence-only, like `ADR:`): absent, the pause silently never fires — the finding-#2/#21/#28 shape, a mechanism that exists and is not invoked. The tier never reads it.
- **The packet's plan-vs-actual comparison gets an output slot.** Assembly step 3 compared the changed-file inventory with `Files likely to change` and had nowhere to put the result, so it could happen silently. The packet now carries `Deviations:` — one entry per out-of-plan file, per planned file left untouched, per slice landed through a different approach than written, and per 8.6 amendment — and `Red pause:` resolving to the quoted release, `skipped at gate`, or `n/a — trivial`. Disclosure is not authorization: out-of-scope work still reopens the gate. `ctdd-review` treats a `none` as verifiable against the diff and an absent field or undisclosed amendment as `process-evidence`; 9.4 hands the diff over with a `--stat` summary in front of it.
- **Rejected rather than filed:** a post-red `.code.md` artifact was drafted for the approach-level gap, then superseded the same day — the pilot reported the failure mode had already occurred, so its trigger was stale on arrival, and its useful content (approach per file) ships as the pause's spoken lines instead. Recorded in the rejections table with a reopen condition: repeated `Deviations:` entries on changes whose pause stated the approach and was released.
- **Phased implementation shipped: `red pause` gains a third value.** Under `phased`, the 7.12 pause also proposes the phase map — groups derived from the planned paths in the repo's own structure, dependency-ordered, no architecture assumed — finalized by the release message; new step 8.7 then implements one group at a time, and each checkpoint presents the group's diff with its evidence: build result, every pin green, and the intended-red set changed only by tests the group's own files serve. Continue on an affirmative message; in-scope corrections re-enter at 8.1, out-of-scope reopens the gate under 8.6, and the packet's `Red pause:` field quotes every release. Drafting defaults: `skip` at small (structurally one group), `pause` at medium, `phased` at large or at three-plus groups — the tier measures risk, the group count measures width, and the pilot's recorded miss was a wide change. Restores what the agent did by instinct around v0.11.0, as a declared decision on the approved line.
- **`docs/companions-routing.md`** records the seven entry paths and when each applies — shapeless idea, unknown cause, stated requirement, isolation, landing, decision-only, and judging — plus the six upstream skills deliberately not vendored and why each would collide. Written because the seams are the part a routing decision gets wrong at the moment of choosing, not later.
- **`ctdd-companions` 0.1.1 — the brainstorming land-grab defused, intent-born (pilot feared the overlap; the file confirmed it).** Upstream's description demands itself before *any* creative work, and its process re-explores and re-classifies what ctdd steps 0–3 do — universal double cost. The vendored trigger is inverted: fire only when nobody can state the requirement in a sentence or two; everything statable routes straight to ctdd-change with zero brainstorm tokens. Both exit seams rewired from writing-plans to ctdd-change, and the spec carries a new duty: everything the change needs goes in the document, because the change workflow never rereads the conversation.
- **`ctdd-companions` scaffolded** — a separate thin plugin vendoring brainstorming, systematic-debugging, using-git-worktrees, and finishing-a-development-branch from obra/superpowers (MIT, pinned commit, attribution file). Each description carries a one-way handoff: designs and diagnoses end where ctdd-change begins, worktree edits still gate, branch finishing runs only after step 10's packet. Separate plugin by design: ctdd's guards, budgets, and tests stay untouched, and upstream re-syncs never touch the change loop.
- **Evolve-or-adopt-Superpowers settled by inspection of the actual repo:** its breadth (brainstorming, debugging, worktrees, branch finishing) fills ctdd-shaped holes and is filed for absorption in Tier 4; its core enforcement is prose and diagrams — no checkers, approval hashes, red-state verification, Deviations, or budgets — the exact material the findings ledger replaced, so the change loop stays ctdd. Evolution itself is now specimen-gated across the whole backlog: the next investment is running changes, under any tool.
- **Subagents-per-step considered and scoped:** the spine stays in-thread — understanding is the workflow's currency, the plan is deliberately an insufficient brief, gates cannot live in forks, and the guardrail prices every fork as a claim requiring inspection. Independence-requiring judgment forks (reviewer, blind eval — both filed); a read-only orientation scout joins Tier 3 for the day a large repo exhausts context during steps 2–3.
- **Second harvest from the shipped skills:** the reviewer-subagent entry upgraded with skill-creator's confirmed disk protocol (agents as plain prompt files, paths in, output path out, zero shared context); blind-comparison with post-hoc unblinding noted in the eval entry for competing skill revisions; and repo-content injection hygiene filed from `import-memory`'s discipline — data never instruction, directives dropped with their set-up sentence, and behavioral effect makes a directive regardless of phrasing. pdf's conditional loader grammar stands as shipping precedent for the steps-7–10 refactor.
- **Filed: four patterns absorbed from Anthropic's shipped skills**, each living outside the exhausted route and body budgets — a read-only reviewer subagent (independence by tool permissions, per skill-creator's bundled `agents/`), post-write checker hooks (findings #2/#21/#28 at the harness level), trigger-accuracy evals with negative-trigger descriptions (skill-creator's benchmark loop, `morning`'s anti-trigger pattern), and a deterministic `/ctdd` command. Evidence labels honest per entry: two ride recorded pain, two wait on their first specimen.
- **The ADR rules were audited against the canon on a live specimen, and four gaps closed.** ADR-0002 was amended in place — a state its own Context can no longer reconcile: it still lists resolve-at-mint as rejected while the rewritten Decision chooses it. Traced to the generator, not the agent alone: rule 16 said "to reverse", so a partial evolution read as out of scope and the agent improvised an amended status (rule 16 now covers alter, in part or in full, and names the reconciliation duty); rule 10 demanded options inside a two-to-five-sentence Context, an instruction that defeats itself past two options (`## Options` is now its own template section, descriptive only — the fillable-or-decline enforcement stays parked in Tier 2 awaiting a fake specimen; this was a good ADR proving options need a home); the categorical line's singular shape nudged two structural decisions into one record (rule 1 now says list every number); and nothing enforced immutability (the reviewer now reads an in-place edit of an accepted ADR's frozen sections as `process-evidence`, mechanically visible in the diff). On pilot pushback — a decision wrong through misreading felt like it deserved correction, not a monument — the Status line gained an optional cause (`Superseded by NNNN — decided on a misread credential`): wrongness gets first-class expression without mutating a record that live commits and `ADR-NNNN` markers already point at. Sub-numbering (`0002.1`) was considered for lineage and rejected: it breaks the directory's chronology, forces a revision-or-new taxonomy call at every supersession — dodgeable homework — and fails many-to-one supersessions; the lineage rides in the title instead, `(supersedes NNNN)`, with numbers staying flat.
- **`ctdd-tests` bans the diff asserting the diff.** A pilot change shipped a config-echo test — `CustomerProfileSettingsTests` reading appsettings values back against the literals the same change wrote, named `IsBoundedByTheConfiguredTimeouts` while asserting only presence. The class is dangerous precisely because it performs the ritual perfectly: red before, green after, by construction, while observing nothing the system does and implying a guarantee the plan's own residual risk had disclaimed. New write-step rule: assert nothing whose only source is an artifact the change writes — assert the behavior the value produces, or record `New-behavior tests: none`; parse and boot failures already fail any host-booting fixture. Review item 2 gains the reverse direction: names claiming more than their assertion observes get renamed to what they check. Body limit 16,000 → 16,400, continuing its recorded ledger: ~175 paid by rationale subtraction, the rest raised against a kept specimen.
- **Drafted, reverted the same day, filed:** an ADR qualification test — `Options` as a required template section fillable three ways (losing alternative, rejected null option, or the forcing constraint with its revisit condition), rule 1 declining `ADR: none` when none fill, rule 14 naming restated plans and implementation details as fakes. Reverted because it shipped against a *feeling*, not a specimen — the exact plausible-story shape this repo rejects — and because forced fields invite manufactured losers (finding #24/#27), which would launder fakes with fake tradeoffs. Filed in Tier 2 gated on a specimen: the next ADR the pilot judges fake, kept and tested against the draft.
- **The route-cost ratchet rose for the first time: 46,000 → 47,000.** Raised honestly rather than gamed: discounting reference files was rejected because plan-format, execution, and worked-change load at least once per plan-gated change, so the route total approximates real context cost wherever text lives. The raise clears the bar every future raise must now meet — recorded pain, subtraction exhausted at 7 characters, and the feature fully specified before the space existed. The reason is written at the constant itself. After landing, 120 characters remain, and the 9.1 probe sits 13 from its ceiling: the next body-side feature triggers the filed steps-7–10 refactor, which stays filed for compaction survival, no longer as a route unblock.
- **The route-cost ratchet was paid by subtraction, not raised.** The feature landed ~1,000 characters of new instruction into a route with ~430 of headroom, so every budget guard fired — the 46,000 ratchet, the 16,000 body proxy, the 13,700 probe ceiling, and the colocated-notes loader falling past the surviving head. Paid the v0.10.4 way: the 7.12 action detail lives in `execution.md`'s break-point table with a one-line stop in the skill, and fourteen sentences of rationale-restating-adjacent-rules came out of the skill body and `plan-format.md` — every rule, probe, and loader verified surviving mechanically. One cross-skill fix: `ctdd-review` may not name `ctdd-change` step numbers, so the deviation check says "undisclosed amendment", not "8.6".

## 0.41.0 — 2026-08-24

- **The plan gains a flow narrative.** A plan carried behavior as one `Intended behavior` sentence plus test bullets, so the reader at the gate could not reconstruct the runtime sequence of the feature or see what changes inside it — the two questions the gate exists to answer. `plan-format.md` adds a `Behavior flow` section directly after `Existing behavior`, mandatory at the medium and large tiers: a `Current flow` derived from the same contract-and-tests reading `Existing behavior` cites, then a `Flow after change` with every step tagged `(unchanged)`, `(changed)`, or `(new)` and each changed step stating what it did before, closing with the adjacent paths left alone. Division of labor is explicit: `Existing behavior` owns *where* current behavior is pinned, the flow owns *what happens*, `Implementation slices` owns the code delta — so the section duplicates neither. `Current flow: none — greenfield` covers step-2 greenfield readings.
- **All three flow headings are checked, not one.** `check-plan.py` requires `behavior flow`, `current flow`, and `flow after change` at medium and above — a subheading required by format prose alone is the finding-#39 failure shape, present parent and silently absent blocks. Large plans now check 22 sections; the small tier omits the section entirely, because small means no new behavior and tiers shrink documentation, never evidence.
