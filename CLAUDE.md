...
**3. Skill prose changes need the human's approval — stop and ask.** The scarce resource is the agent's *attention across rules*, not word count. Before adding or rewording anything in a `SKILL.md` or a reference, say what it displaces, what it costs against the body and route budgets, and what evidence demanded it — then wait. The bar to argue is "a real-use finding demanded it", not "a review suggested it"; reviews are an exhausted source, and three rounds re-proposed each other's rejections. Prefer script-side, reference-side, or a displacement (delete instruction X, replace with mechanical check Y). Where the only honest resolution is a human decision, say so rather than resolving it quietly — a budget guard firing is not permission to drop the cheapest item and call it a displacement. Project-specific framework syntax and conventions belong to the target repository's applicable `CLAUDE.md` / `.claude/rules/`, project configuration, and adjacent tests; keep skill examples framework-neutral unless a CTDD-specific mechanism requires exact syntax.
...
**9. When a budget guard fires, displace or decide — never shave, and never resolve it alone.** Trimming sentences until the number passes breaks the rules the survival probes protect and hides the decision. Move something out, or move the limit and record the reason in the guard beside the number. **Check what the number is first:** the compaction reserve is early warning — the survival probes are the real constraint — and the route ratchet was set to wherever the content happened to be, not derived from anything. If the fire forces dropping something the human asked for, say so and let them choose; deleting the cheapest item is not a resolution.
