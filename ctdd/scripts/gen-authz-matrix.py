...
def assert_readable(spec):
    """Refuse a contract nothing could be read from.

    `paths:` as a list made `operations()` raise AttributeError mid-iteration,
    which exits 1 — the code the docstring reserves for "`--check` found drift",
    so CI was told to regenerate the matrix when the real fault is an unparseable
    spec. Guarding inside the generator was no better: it is lazy, so the raise
    never ran and a clean matrix over zero operations exited 0.
    """
    if not isinstance(spec, dict):
        raise ValueError(f"the contract root must be a mapping, found "
                         f"{type(spec).__name__}")
    paths = spec.get("paths")
    if paths is not None and not isinstance(paths, dict):
        raise ValueError(f"`paths` must be a mapping, found "
                         f"{type(paths).__name__}; nothing could be read")
    ...
    for path, item in (spec.get("paths") or {}).items():
        ...
        for method in HTTP_METHODS:
            ...
            if isinstance(op, dict):
                ...
                # A present-but-unreadable operation (`post: null`, `delete:
                # "TODO"`) was dropped without entering SKIPPED, so the
                # completeness gate never fired and `--check` reported the matrix
                # current over operations it never read.
                print(f"gen-authz-matrix: WARNING — unreadable operation skipped: "
                      f"{method.upper()} {path}", file=sys.stderr)
