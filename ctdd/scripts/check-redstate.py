...
# Most CI runners emit colour by default, and an SGR escape terminates in `m` —
# an alphanumeric — so `\x1b[31mFAILED\x1b[0m` disqualified its own marker. The
# mention survived as a verdict-less line and routed to *passing*: a red test was
# reported as having "passed before implementation", with the remediation text
# accusing a correct test of asserting nothing. Strip escapes before any matching.
_ANSI_RX = re.compile(r"\x1b\[[0-9;?]*[ -/]*[@-~]|\x1b[@-Z\\-_]")
...
def _decode(raw):
    ...
    if raw.startswith(b"\xef\xbb\xbf"):
        return _strip_ansi(raw.decode("utf-8-sig", errors="replace"))
...
# `--- PASS:` is Go's only per-test success line and `ok N - name` is TAP's;
# neither was here, while FAIL_MARKERS carried `fail:` and `not ok`. So both
# runners could report red state and neither could ever verify a pin — the
# lane every change needs. `ok ` self-disqualified too: the character after
# it is the TAP test number, and a digit joins.
PASS_MARKERS = ("passed", "✓", "√", "success", "pass:", "[pass]", "ok")

# A marker glued to anything that can appear inside a test name, a path or a
# module is not a verdict. The first cut disqualified only alphanumerics and `_`,
# so `tests/error/x.py::t PASSED` still voted `error` — a fully green run
# certified as red state, and the pin lane read a FAILED line as a captured
# baseline because `tests/success/…` supplied the pass. Path separators, dots and
# hyphens all join words; a verdict stands alone.
_JOINERS = "_-./\\:"


def _joined(ch):
    return ch.isalnum() or ch in _JOINERS
...
def _marker_positions(low, markers):
    ...
    for m in markers:
        for hit in re.finditer(re.escape(m), low):
            ...
            if _joined(before) or _joined(after):
                continue
            out.append(i)
    return out
...
# The terminator alternation once included a bare `-`, so a hyphenated name
# captured only its first stem — `capture-fails-when-zero` became `capture` —
# and `_match_span` then accepted that stem against *every* hyphenated name
# in the log, because `-` is a boundary. Two distinct tests collapsed to one
# and a single failing run certified both. A separator needs surrounding
# space; a hyphen inside a word does not.
NAME_RX = re.compile(r"\s*[-*]\s+[`*_]{0,3}([A-Za-z][\w.\-]*?[A-Za-z0-9])[`*_]{0,3}(?:\s*(?:—|–|:|\()|\s+-\s|\s*$)")
...
def _is_heading(line, label):
    ...
    # reported success.
    # An indented line continues the bullet above it, whatever its length or
    # punctuation: a wrapped `- name — path: …; case: …` closed the section and
    # the checker verified the shorter list. Indentation is the markdown rule.
    if line[:1].isspace():
        return False
    text = (label or "").strip()
    if not text:
        return False
    # A label is a line that does not read as a sentence. The length ceiling made
    # `plan-format.md`'s own 76-character conditional label fail to close the
    # section, so copying the skeleton verbatim extracted the `n/a` coverage rows
    # as test names and sent the agent hunting for a test that never existed.
...
def names_from_plan(path, want_pins=False):
    ...
    found, skipped, unparsed, section = [], [], [], "other"
    for line in text.splitlines():
        ...
        if section != want:
            continue
        # An indented continuation is prose under a bullet, not another test.
        # `  - note: the boundary is exclusive.` was extracted as the test `note`,
        # and check-redstate then reported it "not found in the log" at 7.11 —
        # after approval and after the tests were written, with no diagnostic
        # naming the cause.
        if line[:1].isspace() and line.lstrip()[:1] in "-*":
            continue
        ...
        if not m:
            # A bullet in a test section that yields no name is a plan defect, not
            # an absence: a fully-qualified `Ns.T::name`, a prose sentence, or a
            # generic like `Capture_Fails<Decimal>` all produced found=[] with
            # nothing reported, so the section read as empty and the tier, the
            # evidence lanes and the packet all agreed there were no tests.
            # Only a *bullet* that yields no name is a defect. Explanatory prose
            # between bullets is legitimate and must not be reported.
            body = line.strip().lstrip("-* ").strip()
            # A bullet that reads as a sentence — spaces before any separator, or
            # ending in terminal punctuation — is explanatory prose, which plans
            # legitimately put in a list. What must not vanish silently is a bullet
            # that *looks like a name and is not one*: a fully-qualified path, a
            # generic parameter, an unbalanced backtick.
            sentence = body.rstrip().endswith((".", "!", "?")) or (
                " " in body.split("—")[0].split(" - ")[0].strip())
            if BULLET_RX.match(line) and body and not sentence:
                unparsed.append(line.strip()[:70])
            continue
    ...
    return list(dict.fromkeys(found)), skipped, unparsed
...
def main():
    ...
    while rest:
        ...
        if flag == "--test" and rest:
            ...
            try:
                _plan = rest.pop(0)
                _found, _skipped, _unparsed = names_from_plan(
                    _plan, want_pins=expect_pass)
                if _unparsed:
                    print(f"check-redstate: {len(_unparsed)} bullet(s) in the "
                          f"evidence section yielded no test name and were "
                          f"dropped: " + "; ".join(_unparsed[:4]))
                    print("    A fully-qualified name, a prose sentence or a "
                          "generic parameter is not a test name here. The section "
                          "would otherwise read as empty, and the tier, the "
                          "evidence lanes and the packet would all agree.")
                    return 2
                if _skipped:
    ...
    if not passing and not absent:
        ...
        if reclassified:
            # Printing "red state verified" and exiting 2 put a verdict the packet
            # copies verbatim (worked-change.md:108) on a run the same command
            # refuses to certify — a conformant packet recording verified red state
            # from an uncertified run. Say the uncertified thing.
            print(f"check-redstate: {len(failing)} of the plan's new test(s) "
                  f"observed failing, but a `currently_`-prefixed name was "
                  f"reclassified out of the set — RED STATE NOT VERIFIED for every "
                  f"test the plan names.")
            return 2
