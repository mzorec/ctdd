...
- Write the tests when `ctdd-change` invokes you at 7.5 or 7.9 under an approved plan: that is the gate having already run, and it is the only route by which a new-behavior test can be written at all.
...
| A `must fail before implementation` test passes | Stop; report whether the behavior already exists or the assertion fails to constrain it. Under an approved plan that is the whole action — return the finding to `ctdd-change`, which owns the production tree at step 7. Standalone, with no plan in flight, you may tell the two apart by breaking the one production rule the test names, re-running, and reverting: green against the broken rule means the test asserts nothing, red means the behavior genuinely exists. Verify the revert by re-running the same command and confirming a clean production diff. |
